...
export function discoverFiles(repoRoot: string): Discovered {
    ...
    if (readResult.error) {
        throw new Error(`Error reading tsconfig.json: ${readResult.error.messageText}`);
    }
    ...
    if (parsed.errors.length > 0) {
        throw new Error(`Error parsing tsconfig.json: ${parsed.errors.map(e => e.messageText).join(", ")}`);
    }
