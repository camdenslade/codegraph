...
export function structuralParse(
	file: string,
	source: string,
): StructuralResult {
...
function handleDeclaration(
	...
	switch (node.type) {
		...
		case "generator_function_declaration": {
			if (!nameNode) return;
			out.push(
				mk("function", nameNode.text, node, exported, docHost, source),
			);
			return;
		}
		case "class_declaration": {
			if (!nameNode) return;
			out.push(
				mk("class", nameNode.text, node, exported, docHost, source),
			);
		...
		case "interface_declaration": {
			if (!nameNode) return;
			out.push(
				mk("interface", nameNode.text, node, exported, docHost, source),
			);
			return;
		}
		...
		case "enum_declaration": {
			if (!nameNode) return;
			out.push(
				mk("enum", nameNode.text, node, exported, docHost, source),
			);
			return;
		}
