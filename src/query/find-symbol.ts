...
export function findSymbol(
  ...
  try {
    ...
    const fileClause = filePart ? "AND file LIKE @fileLike COLLATE NOCASE" : "";
    ...
    if (pool.length === 0) {
      ...
      pool = all.filter((r) => isSubsequence(needle, r.name.toLowerCase()));
    }
    ...
    return pool
      ...
      .sort(
        (a, b) => b.score - a.score || a.qualifiedName.localeCompare(b.qualifiedName),
      )
      .slice(0, limit);
  } finally {
    db.close();
