...
export function getStats(repoRoot: string): Stats {
    ...
    try {
        const nodesByKind = groupCount(db, `SELECT kind, COUNT(*) AS n FROM nodes GROUP BY kind`);
        const edgesByKind = groupCount(db, `SELECT kind, COUNT(*) AS n FROM edges GROUP BY kind`);
        ...
        const resolutionRate = denominator === 0 ? null : resolvedEdges / denominator;
...
function groupCount(db: ReturnType<typeof openDB>, sql: string): Record<string, number> {
